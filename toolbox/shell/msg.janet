###
### msg.janet
###
### A simple message protocol for sequential messages
### over a stream (full duplex channel of strings).
###

# Message Protocol
#
#          | b0 | b1 | b2 | b3 | t1 |  ... n bytes |
#  n = b0 + b1 * 0x100 + b2 * 0x10000 + b3 * 0x1000000
#
# Messages are a four byte little endian unsigned message prefix
# denoting length followed by a one byte type declarator and a payload of length bytes.
# An interrupted or incomplete message should be converted to nil.
# types:
# 0: raw
# 1: json
# 2: msgpack
# 3: jdn
# 4: error

(import spork/json)

(def buf-to-type
  {"\0" :raw
  "\x01" :json
  "\x02" :msgpack
  "\x03" :jdn
  "\x04" :error})

(defn make-recv
  "Get a function that, when invoked, gets the next message from a readable stream.
  Provide an optional unpack function that will parse the received buffer."
  [stream]
  (def buf @"")
  (fn receiver []
    (buffer/clear buf)
    (if-not (:chunk stream 4 buf) (break))
    (def [b0 b1 b2 b3] buf)
    (def len (+ b0 (* b1 0x100) (* b2 0x10000) (* b3 0x1000000)))
    (buffer/clear buf)
    (if-not (:chunk stream 1 buf) (break))
    (def msg-type (get buf-to-type (string buf) (string buf)))
    (buffer/clear buf)
    (if-not (:chunk stream len buf) (break))
    (case msg-type
        :raw (string buf)
        :json (json/decode (string buf))
        :msgpack (error (string/format "type not supported: %j" msg-type))
        :jdn (parse (string buf))
        :error (error (string buf))
        (error (string/format "type not supported: %j" msg-type)))))

(defn make-send
  "Create a function that when called with a msgs sends that msg.
  Provide an optional pack function that will convert a message to a string."
  [stream]
  (def buf @"")
  (fn sender [msg]
    (def x (json/encode msg))
    (buffer/clear buf)
    (buffer/push-word buf (length x))
    (buffer/push buf 1) # 1 for json encoding
    (buffer/push-string buf x)
    (:write stream buf)
    nil))

(defn make-proto
  "Create both a send an recv function from a stream, as with
  `make-send` and `make-recv`."
  [stream]
  [(make-send stream) (make-recv stream)])
