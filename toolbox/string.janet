...
(defn distance [s1 s2]
  (def len1 (length s1))
  (def len2 (length s2))
  (def dp (array/new (inc len1)))
  (loop [i :range [0 (inc len1)]]
    (array/push dp (array/new-filled (inc len2) 0)))
  (loop [i :range [0 (inc len1)]]
    (put-in dp [i 0] i))
  (loop [i :range [0 (inc len2)]]
    (put-in dp [0 i] i))
  (loop [i :range [1 (inc len1)]]
    (def c1 (get s1 (dec i)))
    (loop [j :range [1 (inc len2)]]
      (def c2 (get s2 (dec j)))
      (if (= c1 c2)
        (put-in dp [i j] (get-in dp [(dec i) (dec j)]))
        (let [del (inc (get-in dp [(dec i) j]))
              insert (inc (get-in dp [i (dec j)]))
              substitute (inc (get-in dp [(dec i) (dec j)]))
              minimum (min del insert substitute)]
            (put-in dp [i j] minimum)))))
  (get-in dp [len1 len2]))
