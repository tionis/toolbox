...
void
tb_change_cell(int x, int y, uint32_t ch, uint32_t fg, uint32_t bg)
{
...
static void
write_sgr(uint32_t fg, uint32_t bg)
{
  char buf[32];
  ...
  case TB_OUTPUT_TRUECOLOR:
    ...
    WRITE_LITERAL("m");
    break;
  case TB_OUTPUT_256: /* fallthrough */
  case TB_OUTPUT_216: /* fallthrough */
  case TB_OUTPUT_GRAYSCALE:
    ...
    WRITE_LITERAL("m");
    break;
  case TB_OUTPUT_NORMAL: /* fallthrough */
  default:
    WRITE_LITERAL("\033[");
    if (fg != TB_DEFAULT) {
      WRITE_LITERAL("3");
