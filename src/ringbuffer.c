...
int
init_ringbuffer(struct ringbuffer* r, size_t size)
{
...
void
free_ringbuffer(struct ringbuffer* r)
{
	free(r->buf);
}

void
clear_ringbuffer(struct ringbuffer* r)
{
	r->begin = 0;
	r->end = 0;
}
...
size_t
ringbuffer_free_space(struct ringbuffer* r)
{
...
size_t
ringbuffer_data_size(struct ringbuffer* r)
{
	if (r->begin == 0 && r->end == 0) {
		return 0;
	}
...
void
ringbuffer_push(struct ringbuffer* r, const void* data, size_t size)
{
...
void
ringbuffer_pop(struct ringbuffer* r, void* data, size_t size)
{
...
void
ringbuffer_read(struct ringbuffer* r, void* data, size_t size)
{
	if (ringbuffer_data_size(r) < size) {
		return;
	}
