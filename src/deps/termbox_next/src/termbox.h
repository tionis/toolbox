 ...
 * from a signal handler (SIGWINCH) to the main event reading loop. Honestly
 * in most cases you should just check the returned code as < 0.
 */
...
/* 
 * An event, single interaction from the user. The 'mod' and 'ch' fields
 * are valid if 'type' is TB_EVENT_KEY. The 'w' and 'h' fields are valid
 * if 'type' is TB_EVENT_RESIZE. The 'x' and 'y' fields are valid if 'type' is
